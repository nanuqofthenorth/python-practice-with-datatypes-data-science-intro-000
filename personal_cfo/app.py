...
from cfo import db
from cfo.ui import escape_markdown_dollars, get_api_key, insight_badge, is_ai_configured, render_insights, setup_page, stat_tile
    ...
    try:
        ...
        for item in briefing_items:
            with st.container(border=True):
                ...
                with item_cols[0]:
                    st.markdown(insight_badge(item.get("level", "good")), unsafe_allow_html=True)
                with item_cols[1]:
                    title = escape_markdown_dollars(item['title'])
                    detail = escape_markdown_dollars(item['detail'])
                    st.markdown(f"**{title}**  \n{detail}")
